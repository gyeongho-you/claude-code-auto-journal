  ...
  for (const [project, entry] of entries) {
    ...
    try {
      ...
      if (fs2.existsSync(bakPath)) {
        fs2.copyFileSync(bakPath, hookPath);
        fs2.unlinkSync(bakPath);
        if (process.platform !== "win32") fs2.chmodSync(hookPath, 493);
        console.log(`\u2713 git hook \uBCF5\uC6D0 (\uB808\uAC70\uC2DC): ${project}`);
        continue;
      }
      if (!fs2.existsSync(hookPath)) continue;
      ...
      if (!remaining || remaining === "#!/bin/sh") {
        ...
        fs2.writeFileSync(hookPath, remaining + "\n", "utf-8");
        if (process.platform !== "win32") fs2.chmodSync(hookPath, 493);
