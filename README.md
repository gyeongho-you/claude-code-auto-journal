...
3. `gitCommit.use: true`이면 처음 Claude를 실행한 프로젝트에 git post-commit hook을 자동 등록 — 이후 Claude 없이 직접 커밋할 때도 커밋 메시지와 변경 파일 목록을 기록
4. 매일 지정한 시간(기본 18:00)에 그날의 기록으로 일지(Markdown) 자동 생성 — 대화 기록과 git commit 기록을 함께 참고해 작성
...
---
[커밋 10분] feat: 기능 구현                  ← gitCommit.use: true일 때 포함
 src/feature.ts | 40 ++++
...
[작업]은 Claude와의 대화 기록, [커밋]은 git commit 기록(커밋 메시지 + 변경 파일 목록)이며
...
| `gitCommit.use` | `true` | git commit 기록을 일지에 포함할지 여부. `true`이면 Claude 첫 실행 시 해당 프로젝트에 git post-commit hook 자동 등록 — 이후 Claude 없이 한 커밋도 커밋 메시지·변경 파일 목록이 일지에 반영됨. ⚠️ commit이 많거나 변경 파일이 많을수록 일지 생성 시 입력 토큰이 증가함 (`false`로 설정하면 토큰 절약 가능) |
...
- 이후 커밋 시마다 커밋 메시지와 `git diff-tree --stat` 결과를 기록 (Claude 호출 없음)
- 일지 생성 시 대화 기록과 커밋 기록을 시간순으로 통합 — 대화 직후의 커밋은 해당 대화의 구현 결과로 자연스럽게 연결
...
커밋 기록은 일지 생성 프롬프트에 그대로 포함됩니다. 하루에 커밋이 많거나 변경 파일이 많을수록 입력 토큰이 늘어납니다. 커밋이 잦은 프로젝트에서는 비용이 체감될 수 있으므로 `gitCommit.use: false`로 비활성화하거나, `journal.claudeModel`을 `"haiku"`(기본값)로 유지하는 것을 권장합니다.
