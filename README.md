...
1. Claude Code 세션이 종료될 때마다 마지막 응답을 저장 (기본적으로 답변 내용은 저장, `summary.use: true`면 요약본 생성)
...
Stop 훅과 Task Scheduler가 재등록됩니다. `user-config.json`은 이미 존재하면 기존 값을 유지하면서 누락된 설정만 기본값으로 추가됩니다.
